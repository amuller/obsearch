...
public class OBPriorityQueueShort<O extends OBShort> extends AbstractOBPriorityQueue<OBResultShort<O>> {
	public void add(int id, O obj, short distance) throws InstantiationException, IllegalAccessException {
        if (queue.size() == k) {
            // otherwise we recycle objects!
            if (queue.peek().getDistance() >= distance) {// biggest object in
                                                            // the heap is
                                                            // bigger than d
            	OBResultShort<O> c = queue.poll();
        	...
        	OBResultShort<O> c = new OBResultShort<O>();
        	c.setDistance(distance);
            c.setObject(obj);
            c.setId(id);
            queue.offer(c);
     ...
     */
    public short updateRange(short r) {
        ...
        if (this.getSize() == k) {
            short d = queue.peek().getDistance();
            if (d < r) {
                return d; // return d 
            }           
        }
     ...
     * @return
     */
    public boolean isCandidate(short d){
    	if(this.getSize() == k ){

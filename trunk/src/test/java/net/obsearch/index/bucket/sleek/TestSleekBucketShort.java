...
public class TestSleekBucketShort {
	...
	public void testBucketAux(int maxObjects, int maxPivots)
		...
		SleekBucketShort<L1> bucket = new SleekBucketShort<L1>(L1.class,
				maxPivots);
		int i = 0;
		List<L1> objects = new LinkedList<L1>();
		while (i < maxObjects) {
			L1 o = generateObject();
			if(! objects.contains(o)){
				bucket.insert(null, o);
				objects.add(o);
				i++;
			}
		}
		...
		testUpdateOperations(objects, bucket);
		testUpdateOperations(objects, deSerializedBucket);
		// end result should be the same.
		assertEquals(bucket, deSerializedBucket);
		
	}
	
	private void testUpdateOperations(List<L1> objects, SleekBucketShort<L1> bucket) throws IllegalIdException, OBException, IllegalAccessException, InstantiationException{
		// check size.		
		...
		for(L1 o : objects){
			...
			assertTrue(bucket.exists(null, o).getStatus() == Status.EXISTS);
		}
		...
		for(L1 o : objects){
			// all objects should be inside.
			assertTrue(bucket.delete(null, o).getStatus() == Status.OK);
		}
		...
		for(L1 o : objects){
			// all objects should be inside.
			assertTrue(bucket.insert(null, o).getStatus() == Status.OK);
		}
		assertEquals(bucket.size(), objects.size());
		...
		for(L1 o : objects){
			// all objects should be inside.
			assertTrue(bucket.insert(null, o).getStatus() == Status.EXISTS);
		}

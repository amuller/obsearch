...
public abstract class AbstractTentaclePivotSelector<O extends OB> implements
	...
	public void generatePivots(AbstractPivotIndex<O> x) throws OBException,
			IllegalAccessException, InstantiationException, DatabaseException, PivotsUnavailableException {
		x.prepareFreeze();
		O prev = obtainD(x);
		int m = x.getMaxId();
	...
	/**
	 * Decreases the current value of D (makes it potentially easier
	 * to find pivots to satisfy)
	 ...
	 */
	protected boolean withinRangeAll(O current, O[] obs, int i) throws OBException {
		int cx = 0;
		while (cx < i) {
	 ...
	 */
	protected abstract O obtainD(AbstractPivotIndex<O> x)throws InstantiationException , IllegalAccessException, DatabaseException, OBException;

...
public class SpaceTreeNode implements SpaceTree {
    ...
    public void searchRange(float[][] query, List < SpaceTreeLeaf > result) {
        ...
        /*
         * if(query[DD][MAX] < DV){ left.searchRange(query, result); }else if
         * (query[DD][MIN] >= DV){ right.searchRange(query, result); }else{ //
         * our query has values that belong to both branches... // nothing we
         * can do here, we have to explore everything. left.searchRange(query,
         * result); right.searchRange(query, result); }
         */
        if (query[DD][MIN] <= DV) {

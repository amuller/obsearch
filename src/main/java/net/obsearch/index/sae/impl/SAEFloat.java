package net.obsearch.index.sae.impl;
...
import net.obsearch.index.sae.AbstractSAE;
...
public class SAEFloat<O extends OBFloat> extends AbstractSAE<O> implements IndexFloat<O> {
	
	private Computation[][] matrix;
	private Random r = new Random();
	protected static Logger logger = Logger.getLogger(SAEFloat.class
			.getName());
	public SAEFloat(Class<O> type, int expectedSize) {
	...
	private List<Integer> saeSearch(OBQueryFloat<O> query) throws OBException, InstantiationException, IllegalAccessException{
		...
		return saeSearchAux(query, currentPivot, computed, active);
	}
	
	private List<Integer> saeSearchAux(OBQueryFloat<O> query, int currentPivot, BitSet computed, BitSet active) throws OBException, InstantiationException, IllegalAccessException{
		
		if(active.cardinality() > 0){
			float dist = distance(query, currentPivot, active, computed);
			// from all the active guys, separate the set in guys who are
			// bigger than the current pivot, and guys who are smaller than the current pivot
			BitSet left = new BitSet(objects.size());
			BitSet right = new BitSet(objects.size());
